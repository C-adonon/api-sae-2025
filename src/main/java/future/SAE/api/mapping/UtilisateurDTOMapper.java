...
public interface UtilisateurDTOMapper {
    ...
    @AfterMapping
    default void ajouterRole(Utilisateur utilisateurSource, @MappingTarget UtilisateurReponse reponseCible) {
        if (utilisateurSource != null && reponseCible != null) {
            reponseCible.setRole(utilisateurSource.getClass().getSimpleName().toUpperCase());
        }
    }
}