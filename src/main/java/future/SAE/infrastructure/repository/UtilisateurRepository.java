...
public interface UtilisateurRepository extends JpaRepository<UtilisateurJPA, UUID>
{
    ...
    Optional<UtilisateurJPA> findByIdentifiant(Long identifiant);
    ...
    @Query("SELECT p FROM ProfesseurJPA p");
    List<ProfesseurJPA> findAllProfesseur();

    Optional<ProfesseurJPA> findByIdentifiantAndRole(Long identifiant, String role);

    @Query("SELECT e FROM EleveJPA e");
    List<EleveJPA> findAllEleve();
